...
#define FLASH_DIR_PRODUCCION  0x0020
#define FLASH_DIR_TIEMPOS     0x00A0
DueFlashStorage FLASH;

void initFLASH(){
  uint32_t codeRunningForTheFirstTime;
  if(FLASH.get(FLASH_DIR_DEFAULT, codeRunningForTheFirstTime) != EPOCH_COMPILE)
    defaultFLASH();
  loadFLASH();
}
...
void setStringPRODUCTO(String Cadena){
  ...
  byte bufCadena[lenCadena + 1];
  // Convierte en array de Bytes un String
  Cadena.getBytes(bufCadena,lenCadena+1);
  ...
  if ((lenCadena > 0) && (lenCadena <=MAX_LEN_PRODUCTO)){
    ...
    for(i=0;i<lenCadena;i++){
      produccion.Producto[i]=bufCadena[i];
      if(bufCadena[i] == 0) i=lenCadena;
    }
...
void getFullTimeSTR(){
  /*
  tiempoACTUAL ="";
  tiempoACTUAL += (myRTC.getDOWStr(FORMAT_SHORT) +", ");
  tiempoACTUAL += (myRTC.getDateStr(FORMAT_LONG,FORMAT_LITTLEENDIAN,'/') +" ");
